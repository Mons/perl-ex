...
use Data::Dumper;
my %data;
for (@GD::Barcode::DataMatrix::Engine::FORMATS) {
	$data{$_->[9]} = $_->[10];
}
...
for (sort { $a <=> $b } keys %data) {
	my @orig = my @data = map { int rand 255 } 0..$_-1;
	my $new = GD::Barcode::DataMatrix::Engine::CalcReed(\@data,$data{$_});
	{
		local $" = ',';
		print "is_deeply rs([@orig],$data{$_}),[@$new], '$_/$data{$_}';\n";
	}
	#last;
}
